...
public class GraphsAPI extends API {
    ...
    public Map<String, GraphMode> mode(@Context GraphManager manager,
                                       @PathParam("name") String name,
                                       JsonMode jsonMode) {
        LOG.debug("Set mode to: '{}' of graph '{}'", jsonMode, name);

        E.checkArgument(jsonMode != null && jsonMode.mode != null,
                        "Graph mode can't be null");
        HugeGraph g = graph(manager, name);
        GraphMode mode = GraphMode.valueOf(jsonMode.mode);
        g.mode(mode);
